...
(function() {
  var Telemachus, addTelemetry, calculateTicks, celestialBodies, charts, customCharts, customLayouts, dateString, defaultLayout, drawChartAxes, drawChartGrid, drawChartLegend, drawChartSeries, durationString, hourMinSec, layouts, missionTimeString, orderOfMagnitude, reloadLayout, removeTelemetry, resetChart, resources, setChart, setLayout, siUnit, standardCharts, standardLayouts, stripInsignificantZeros,
    __slice = [].slice;
  ...
  Telemachus = {
    ...
    format: function(value, api) {
      if (!(value != null)) {
        ...
        return this.formatters[this.api[api].units.toLowerCase()](value);
      }
    },
    subscribe: function() {
    ...
    loadAPI: function() {
      ...
      return $.get("datalink", {
        ...
        var api, i, r, resourceApi, _i, _j, _k, _len, _len1, _ref, _ref1;
        _ref = JSON.parse(data).api;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          api = _ref[_i];
          if (api.apistring.match(/^b\./)) {
            for (i = _j = 0, _ref1 = celestialBodies.length; 0 <= _ref1 ? _j < _ref1 : _j > _ref1; i = 0 <= _ref1 ? ++_j : --_j) {
              _this.api[api.apistring + ("[" + i + "]")] = api;
            }
          } else if (api.apistring.match(/^r\./)) {
            if (api.apistring !== "r.resourceCurrent") {
              for (_k = 0, _len1 = resources.length; _k < _len1; _k++) {
                r = resources[_k];
  ...
  $(document).ready(function() {
    var $chartMenus, $layoutMenu, chart, populateLayoutMenu, savedDefault, _ref, _ref1;
    if (window.localStorage != null) {
      customCharts = (_ref = JSON.parse(window.localStorage.getItem("customCharts"))) != null ? _ref : {};
      $.extend(charts, standardCharts, customCharts);
      customLayouts = (_ref1 = JSON.parse(window.localStorage.getItem("customLayouts"))) != null ? _ref1 : {};
    ...
    Telemachus.loadAPI().done(function() {
      $("#apiCategory").change();
      return reloadLayout();
    });
